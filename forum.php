...
if (!empty($_GET)) {
	...
	if ($getThread !== false) {
		...
		if ($threadData !== false) {
			...
			if ($access) {
				$threadPlayer = ($config['forum']['outfit_avatars'] || $config['forum']['player_position']) ? mysql_select_single("SELECT `id`, `group_id`, `sex`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype` FROM `players` WHERE `id`='".$threadData['player_id']."';") : false;
				?>
				...
				<table class="znoteTable ThreadTable table table-striped">
					...
					<tr>
						...
						<td class="avatar">
							...
							<br><img src="<?php echo $config['show_outfits']['imageServer']; ?>?id=<?php echo $threadPlayer['looktype']; ?>&head=<?php echo $threadPlayer['lookhead']; ?>&body=<?php echo $threadPlayer['lookbody']; ?>&legs=<?php echo $threadPlayer['looklegs']; ?>&feet=<?php echo $threadPlayer['lookfeet']; ?>" alt="img">
							<?php endif; ?>
				...
				if ($posts !== false) {
					...
					if ($config['forum']['outfit_avatars'] || $config['forum']['player_position']) {
						...
						$sql_players = mysql_select_multi("SELECT `id`, `group_id`, `sex`, `lookbody`, `lookfeet`, `lookhead`, `looklegs`, `looktype` FROM `players` WHERE `id` IN (".implode(',', array_keys($players)).");");
						...
						<table class="znoteTable ThreadTable table table-striped">
							...
							<tr>
								...
								<td class="avatar">
									...
									<?php if ($config['forum']['outfit_avatars']): ?>
									<br><img src="<?php echo $config['show_outfits']['imageServer']; ?>?id=<?php echo $players[$post['player_id']]['looktype']; ?>&head=<?php echo $players[$post['player_id']]['lookhead']; ?>&body=<?php echo $players[$post['player_id']]['lookbody']; ?>&legs=<?php echo $players[$post['player_id']]['looklegs']; ?>&feet=<?php echo $players[$post['player_id']]['lookfeet']; ?>" alt="img">
									<?php endif; ?>
									<?php if ($config['forum']['player_position']): ?>
